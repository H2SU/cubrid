  ...
  if (op == PT_RANGE)
	...
	{			/* has only one range */
	  ...
	  if (opd1 && opd1->node_type == PT_VALUE && opd2 && opd2->node_type == PT_VALUE)
	   ...
	   op == PT_BIT_TO_BLOB || op == PT_BLOB_FROM_FILE || op == PT_CHAR_TO_BLOB ||
	   op == PT_BLOB_TO_BIT || op == PT_BLOB_LENGTH || op == PT_CHAR_TO_CLOB || op == PT_CLOB_TO_CHAR ||
	   op == PT_CLOB_FROM_FILE || op == PT_CLOB_LENGTH || op == PT_BLOB_TO_BFILE ||
	   op == PT_BFILE_TO_BLOB || op == PT_CLOB_TO_CFILE || op == PT_CFILE_TO_CLOB ||
