...
query_is_internal_lob_dml_source (const DB_VALUE *value)
{
  ...
  return marker == DB_VALUE_INTERNAL_LOB_MARKER_NONE
    || marker == DB_VALUE_INTERNAL_LOB_MARKER_FILE_SOURCE
    || marker == DB_VALUE_INTERNAL_LOB_MARKER_PENDING;
}
...
query_initialize_internal_lob_dml_input (const DB_VALUE *value, INTERNAL_LOB_DML_INPUT *input)
{
  ...
  if ((marker == DB_VALUE_INTERNAL_LOB_MARKER_FILE_SOURCE
	&& !query_parse_internal_lob_file_source (value, input))
      || (marker == DB_VALUE_INTERNAL_LOB_MARKER_PENDING
	  && !query_parse_internal_lob_pending_source (value, input)))
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1,
  ...
  if (read_bytes != request_size)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_ES_GENERAL, 2, "LOB",
	      "unexpected end of external LOB source");
      return ER_ES_GENERAL;
    }
				       ...
				       const OID *direct_class_oid)
{
  ...
  *slot_count_out = 0;
  ...
  if (slot_count > 1024)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1,
	      "too many internal LOB DML payload slots");
      return ER_STREAM_SESSION_ERROR;
    }
  ...
  slot_configs =
    (internal_lob_dml_slot_config *) malloc ((size_t) slot_count * sizeof (internal_lob_dml_slot_config));
				      ...
				      const OID *direct_class_oid)
{
  ...
  *stream_values_out = NULL;
  ...
  error = internal_lob_dml_from_init (&null_xasl_id, 0, NULL, (QUERY_FLAG) 0, &cache_time, 0, slot_configs,
				      slot_count);
  if (error != NO_ERROR)
    {
      goto exit;
				      ...
				      const OID *direct_class_oid)
{
  ...
  *list_idp = NULL;
  ...
  if ((flag & RETURN_GENERATED_KEYS) != 0 || var_cnt <= 0 || varptr == NULL)
    {
