			  ...
			  int oos_attrid)
{
  ...
  if (log_type == LOG_REPLICATION_DATA)
      ...
      if (heap_get_class_tde_algorithm (thread_p, class_oid, &tde_algo) != NO_ERROR)
	{
      ...
      if (is_oos_repl)
	{
	  repl_rec->length += 2 * OR_INT_SIZE;	/* metadata magic and target attrid */
	}
...
repl_log_insert_oos (THREAD_ENTRY * thread_p, const OID * class_oid, const OID * inst_oid,
		     LOG_RCVINDEX rcvindex, DB_VALUE * key_dbvalue, REPL_INFO_TYPE repl_info, int attrid)
{
  ...
  if ((rcvindex != RVREPL_OOS_INSERT && rcvindex != RVREPL_INTERNAL_LOB_INSERT
    ...
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_REPL_ERROR, 1,
	      "invalid typed OOS replication metadata");
