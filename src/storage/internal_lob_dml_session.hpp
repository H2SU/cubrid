 ...
 *
 * PoC note: each slot currently stages its complete payload in one contiguous
 * server-memory buffer. This intentionally trades bounded memory for removing
 * the tmpfile write/read pass and is not a production memory policy.
 */
class internal_lob_dml_staging_context : public internal_lob_dml_context
{
  ...
  private:
    ...
    static int read_slot (void *ctx, char *buffer, int buffer_size, int *nread);
    int reserve_slot (slot_state &slot, DB_BIGINT required_size);
