...
{
  VFID lob_vfid;
  FILE *spill_file = NULL;     /* append-order staging; consumed tail->head at end */
  std::string spill_path;
  char *segment_buffer = NULL; /* max internal_lob_segment_size + head header */
  int segment_size = 0;        /* payload bytes per LOB chain chunk */
  DB_BIGINT total_bytes = 0;   /* physical payload bytes staged */
};
...
struct internal_lob_reader
{
  ...
  OOS_READER oos_reader;       /* reader over the current LOB node payload */
  DB_BIGINT logical_length = 0;
