...
slocator_find_lockhint_class_oids (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  if (found_lockhint != NULL)
    {
      assert (found_lockhint->packed == NULL);
    }
...
#if !defined(WINDOWS)
  error = memmon_disabled_force ();
  memmon_disabled = true;
  unsetenv ("LD_PRELOAD");
...
/*
 * smmon_dump_memory_usage - dump memory usage
 */
void
smmon_dump_memory_usage (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  char *ptr;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  ...
  if (!memmon_disabled)
    {
      error = memmon_dump_memory_usage ();
    }
